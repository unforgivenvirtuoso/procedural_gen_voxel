...
# this script was written by codat https://www.youtube.com/watch?v=tgkGysJnHbs
# i tinkered with some values in order to achieve the results i wanted 
...
func _init(leaves = null):
	...
	randomize()
	mesh.radial_segments = rand_range(4, 10)
	mesh.height = rand_range(8, 10)	
	mesh.top_radius = rand_range(0.1, 0.6)
	mesh.bottom_radius = rand_range(mesh.top_radius, 0.7)
	...
	noise.period = rand_range(8, 512)
	var ampl = rand_range(0.2, 2.6)
